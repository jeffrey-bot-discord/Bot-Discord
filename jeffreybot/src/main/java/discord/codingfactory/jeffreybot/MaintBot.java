...
public class MaintBot {
    ...
    MaintBot(String token) throws IllegalArgumentException, RateLimitedException, InterruptedException{
        try{
            jda = new JDABuilder(AccountType.BOT).setToken(token).setGame(of("Type !cmd")).addEventListener(new MyListener()).buildBlocking();
